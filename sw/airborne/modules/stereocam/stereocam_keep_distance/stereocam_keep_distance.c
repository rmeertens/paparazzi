 ...
 */
/**
 * @file "modules/stereocam/stereocam_keep_distance/stereocam_keep_distance.c"
 * @author Roland
 * Keeps kistance to a person based on the stereocam data.
 */
...
void stereocam_keep_distance_periodic()
{
  ...
  if (stereocam_data.fresh && stereocam_data.len > 20) {
    ...
    // When in ATT mode we safe some values for when we enter the
    ...
    // Simple control for now...
    float pitchToTake = -0.08;
    ...
    // Only set the pitch and roll one every four frames... as the control influences the measurements
    if (counterStab % 4 == 0) {
    ...
    // move the heading to look at a goal object
    nav_set_heading_deg(headingStereocamStab);

    // Add a pitch compensation (detemined when going to module mode)
    ref_pitch += pitch_compensation;
    ref_roll += roll_compensation;

    // bind the pitch to a max safe value
    ...
    // bind the ref roll to a max safe value
    if (ref_roll > 0.15) {
      ref_roll = 0.15;
...
void guidance_h_module_run(bool in_flight)
{
  ...
  // Add a yaw form the joystick for quick takeover in module mode
  float yawRate = 0.2 * ((float)radio_control.values[RADIO_YAW]) / ((float)MAX_PPRZ);
  headingStereocamStab += yawRate;

  // Add a roll form the joystick for quick takeover in module mode
  float addedRollJoystick = 0.4 * ((float)radio_control.values[RADIO_ROLL]) / ((float)
                            MAX_PPRZ);
  ...
  // Add a pitch form the joystick for quick takeover in module mode
  float addedPitchJoystick = 0.4 * ((float)radio_control.values[RADIO_PITCH]) / ((float)
                             MAX_PPRZ);
  ...
  command.theta = ANGLE_BFP_OF_REAL((-1.0 * ref_pitch) + addedPitchJoystick); // -1 for reversed lisa
