...
void stereocam_keep_distance_periodic()
{
  ...
  if (stereocam_data.fresh && stereocam_data.len>20) {
    ...
    DOWNLINK_SEND_STEREO_VELOCITY(DefaultChannel, DefaultDevice, &closest,&dist, &guidoVelocityHor,&guidoVelocityHor,&guidoVelocityZ,&current_state,&totalStabiliseStateCount,&stabilisationLateralGains.pGain);
  }
}
